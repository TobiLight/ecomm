...
import { a as uploadSharp, d as deleteFile } from "../../../../../../../chunks/index5.js";
...
async function categories() {
  ...
  for await (const category of data) {
    const imagePath = getFileInCurrentDirectory$1(
      `category/${category.image}.svg`
    );
    const image = sharp(imagePath);
    category.image = await uploadSharp(image, "category");
  }
  const repository2 = useRepository("category");
  const items = await repository2.createMany(data);
  items.shift();
  return items;
}
function getFileInCurrentDirectory$1(filename) {
...
async function products(categories2) {
  const data = [];
  for await (const [categoryIndex, category] of productsData.entries()) {
    for await (const product of category) {
      ...
      const categoryId = categories2[categoryIndex].id;
      data.push({ ...product, image, categoryId });
    }
  }
  const repository2 = useRepository("product");
  return repository2.createMany(shuffleArray(data));
}
