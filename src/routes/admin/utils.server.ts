...
export async function createUser(input: z.infer<typeof userSchema>): Promise<User | null> {
  ...
  try {
    return await repo.create({...input, password, createdAt: new Date(), updatedAt: new Date()})
  } catch(err: any) {
