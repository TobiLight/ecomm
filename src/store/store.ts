...
export let categoriesArr: Writable<Array<{ id: string; name: string }>> =
  writable<Array<{ id: string; name: string }>>([]);
...
export function removeCategory(categoryId: string) {
  return categoriesArr.update((category) =>
    category.filter((cat) => cat.id !== categoryId),
  );
}

export function setCatrtCount(size: number) {
  cartCount.set(size);
}
